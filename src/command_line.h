 ...
 *
 * ```
 * add_arg('a', "add", "Specify using the 'add' operation.");
 * add_arg('n', "num-widgets", "The number of widgets that should appear.");
 *
 * parse_opts(argc, argv);
 *
 * bool add = get_is_set('a');
 *
 * int num_widgets = -1;
 * if (get_is_set('n')) {
 *   num_widgets = atoi(get_value('n'));
 * }
 *
 * if (num_widgets <= 0) {
 *   num_widgets = DEFAULT_VALUE;
 * }
 * ```
 */
